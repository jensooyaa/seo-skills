...
3. **不要自己动手翻页面。** 需要的事实「页面概况」里都给了。不要再用 web_fetch、
   curl、浏览器、正则去补抓 —— 手工抓的结果不稳定、不可复现，还会写出
   「具体值需确认」这种自己都没确认下来的话。页面概况里没有的，就说没有。
4. **只说你真的知道的。** 不能说「结构化数据配置完善」（那组规则没实现，只列了
   有哪些 `@type`，一个字段都没校验）、不能说「alt 与图相符」（你看不到图）、
   不能说「没有被 noindex」（索引指令组没实现，`X-Robots-Tag` 根本没查）。
